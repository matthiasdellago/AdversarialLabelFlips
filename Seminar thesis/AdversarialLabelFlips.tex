...
\begin{abstract}
	...
	We discover that this semantically similar classes are more likely to be confused with another, leading us to hypothesise that the convolutional neural networks recognise these similarities.
\end{abstract}
...
Targeted attacks aims to cause a misclassification into a specific target class. For example, all non-dog images should be misclassified as a dog.
Untargeted attacks only try to evade correct classification. 
...
We will present our results in terms of confusion matrices, where Figure \ref{fig:matrixexample} serves as an example. For large matrices numbers become more difficult to grasp, so we will display our results in heatmap-style images, as seen on the title page.
...
Since adversarial examples have been first introduced in \cite{Szegedy13} a large body of literature has flourished on this topic. We in the following we will briefly introduce the attacks we use.
