...
\begin{frame}[fragile]
	...
	\begin{columns}
		\begin{column}{.5\columnwidth}
			...
			\end{block}
			\only<1-2>{\begin{alertblock}{Fast gradient sign method}
					FGSM is a very fast an simple attack, which was introduced in \cite{goodfellow2014explaining}.
					\end{alertblock}}
			\only<3->{\begin{alertblock}{Projected gradient descent}
					Projected gradient descent (PGD) is a popular, strong attack, which iteratively computes FGSM \cite{madry2017towards}.
				\end{alertblock}}
		\end{column}
	\visible<2->{
		\begin{column}{.5\columnwidth}
